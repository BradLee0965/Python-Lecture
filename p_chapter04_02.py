...
print(id(l)) # 튜플은 불변 타입, 선언될 때 마다 새로운 메모리 확보해서 id 할당시킴. 
print(id(m)) # id 값 변화 없음. 리스트는 가변타입이기때문에 할당된 메모리에서 수정가능하다. 
...
print('sorted -', sorted(f_list, key=len)) # 길이별로 정렬
print('sorted -', sorted(f_list, key=lambda x: x[-1])) # 단어 끝자 알파벳 순으로 반환
